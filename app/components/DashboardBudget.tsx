...
export default function DashboardBudget({ budgets, categoryTotals }: DashboardBudgetProps) {
  ...
  return (
    <div className="card p-2 h-full flex flex-col" role="region" aria-label="Pelacakan Anggaran Bulanan">
      <div className="flex items-center justify-between mb-2 shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <h3 className="text-xs font-semibold text-white">Anggaran</h3>
            {hasAlert && (
              <p className="text-[9px] text-amber-400 flex items-center gap-0.5">
                <Bell className="w-2.5 h-2.5" /> Hampir penuh
              </p>
            )}
          </div>
        </div>
      ...
      <div className="space-y-2 flex-1 overflow-y-auto">
        {budgets.length === 0 && !isAdding && (
          <div className="flex flex-col items-center justify-center py-4 text-center">
            ...
            <p className="text-xs text-zinc-400">Belum ada anggaran</p>
            <p className="text-[10px] text-zinc-600">Klik +</p>
          </div>
        )}
        ...
        {budgets.map((budget) => {
          ...
          return (
            <div key={budget.id} className="p-2 rounded-lg bg-[#1f1f23] border border-zinc-800/50">
        ...
        {isAdding && (
          <div className="p-2 rounded-lg border border-indigo-500/30 bg-indigo-500/5 space-y-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#1f1f23] border border-zinc-800 text-white rounded-lg text-xs p-2 focus:border-indigo-500 outline-none"
              />
              <input
                type="text"
                placeholder="Limit"
                ...
                className="w-24 bg-[#1f1f23] border border-zinc-800 text-white rounded-lg text-xs p-2 focus:border-indigo-500 outline-none"
              />
            </div>
            <div className="flex gap-2">
              <button
